...
func TestHandleMetadataRequestWithoutFields(t *testing.T) {
	f := &proxy.Flow{
		Request: &proxy.Request{
			URL: &url.URL{
				RawQuery: "kind=items&alt=json",
			},
		},
	}
	...
	expectedQuery := "alt=json&kind=items" // Expecting only alt=json, fields is removed.
